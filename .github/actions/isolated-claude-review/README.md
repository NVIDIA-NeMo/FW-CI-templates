...
- `_isolated_review_analyze.yml` runs a full-SHA-pinned Claude Code Base Action without GitHub/OIDC permission, a proposed-code checkout, or publication tools. The action starts in an empty trusted directory, uses bare and restricted modes with strict MCP configuration, disables project/local settings, hooks, bundled skills, slash commands, plugins, session persistence, and built-in shell, filesystem, web, mutation, and subagent tools, and exposes only the bounded audited `review_context` MCP server. The checkout is pinned and the workflow verifies that its worktree remains unchanged before invoking it. Only the validated schema-constrained result and retrieval audit are uploaded; the execution transcript is not published.
